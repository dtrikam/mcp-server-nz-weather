// filepath: /Users/dipeshtrikam/Documents/mcp-server-nz-weather/src/weather-utils.ts
...
export async function makeOpenMeteoRequest(
  latitude: number,
  longitude: number
): Promise<any | null> {
  try {
...
export function isLikelyUS(latitude: number, longitude: number): boolean {
...
export function formatOpenMeteoForecast(data: any): string {
  if (!data || !data.daily) {
...
export const nzCities: Record<string, { latitude: number; longitude: number }> =
  {
...
// Create a configured MCP server instance with weather tools
